...
*****************************************  IMPORTANT NOTE  **********************************/
  ...
  /*! \mainpage Pure File Magic (PFM) API
       ...
       index file.  The files will be "huge" files.  That is, they will actually be one
       or more files up to 2GB in size that are handled "transparently" by the "huge" I/O
       package.  The "huge" I/O package was written prior to Microsoft Windows being able 
       to support files larger than 4GB.<br><br>
       ...
       The PFM I/O library is not thread safe, however, if you do not used cached I/O and you use separate
       PFM handles it is thread safe for reading.  In other words, you can open the same PFM structure using
       multiple handles (open the same file multiple times) and perform reads in threads without having
       a collision problem.
...
#define         PING_BITS                   32   /*!<  Default number of bits used for ping/record number  */
#define         BEAM_BITS                   16   /*!<  Default number of bits used for beam/subrecord number  */
