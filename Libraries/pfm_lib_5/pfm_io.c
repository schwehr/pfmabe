...
void ivs_double_bit_pack (NV_U_BYTE *, NV_U_INT32, NV_U_INT32, NV_INT64);
NV_U_INT64 ivs_double_bit_unpack (NV_U_BYTE *, NV_U_INT32, NV_U_INT32);
...
static void (*pfm_dbl_bit_pack[2]) (NV_U_BYTE [], NV_U_INT32, NV_U_INT32, NV_INT64) = {pfm_double_bit_pack, ivs_double_bit_pack};
static NV_U_INT64 (*pfm_dbl_bit_unpack[2]) (NV_U_BYTE [], NV_U_INT32, NV_U_INT32) = {pfm_double_bit_unpack, ivs_double_bit_unpack};
...
NV_INT32 read_bin_header (NV_INT32 hnd, BIN_HEADER *head)
{
    ...
    hfseek (bin_handle[hnd], 0, SEEK_SET);

    if (!(i = hfread (bin_header_block[hnd], BIN_HEADER_SIZE, 1, bin_handle[hnd])))
      {
...
NV_INT32 write_bin_header (NV_INT32 hnd, BIN_HEADER *head, NV_BOOL init)
{
    ...
    if (init)
    {
        ...
        for (i = 0 ; i < bin_header[hnd].bin_height ; i++)
        {
            ...
            {
                hfwrite (bin_record_data[hnd], bin_off[hnd].record_size, 1, bin_handle[hnd]);
            }
        ...
        */
        ...
        hfseek (bin_handle[hnd], hd[hnd].coverage_map_address, SEEK_SET);
        ...
        for (i = 0 ; i < bin_header[hnd].bin_height ; i++)
        {
            hfwrite (cov, bin_header[hnd].bin_width, 1, bin_handle[hnd]);
    ...
    hfwrite (bin_header_block[hnd], BIN_HEADER_SIZE, 1, bin_handle[hnd]);
...
static NV_INT32 write_bin_buffer_only (NV_INT32 hnd, NV_INT64 address)
{
    ...
    if (!(hfwrite (bin_record_data[hnd], bin_off[hnd].record_size, 1,
        bin_handle[hnd])))
    {
...
static NV_INT32 open_bin (NV_INT32 hnd, NV_CHAR *path, BIN_HEADER *head)
{
    ...
    if ((bin_handle[hnd] = hfopen (path, "rb+")) < 0)
    {
        ...
        else
        {
            ...
            if (errno == EEXIST || (bin_handle[hnd] = hfopen (path, "wb+")) < 0)
            {
                sprintf (pfm_err_str, "Unable to open bin file %s", path);
                return (pfm_error = OPEN_BIN_OPEN_ERROR);
            }
...
static NV_INT32 write_depth_buffer (NV_INT32 hnd, NV_INT64 address)
{
    ...
    if (!(hfwrite (depth_record_data[hnd], dep_off[hnd].record_size, 1,
        index_handle[hnd])))
    {
...
/*!
  ...
  - Purpose:            Opens the index (huge) file.
...
static NV_INT32 open_index (NV_INT32 hnd, NV_CHAR *path)
{
    ...
    if ((index_handle[hnd] = hfopen (path, "rb+")) < 0)
    {
        /*  Added this section of code to try to open read-only if we get a permission denied error.
            This can happen if the file exists but can't be written.  If it can't be read it will
            still fail.  JCD 06/22/11  */
        ...
        {
            if ((index_handle[hnd] = hfopen (path, "wb+")) < 0)
            {
                sprintf (pfm_err_str, "Unable to open index file %s", path);
                return (pfm_error = OPEN_INDEX_OPEN_ERROR);
            }
...
/*!
  ...
  - Purpose:            Closes the index (huge) file.
...
static void checkpoint_pfm_file (NV_INT32 hnd)
{
    ...
    index_size = hftell (index_handle[hnd]);
    hfseek (index_handle[hnd], 0, SEEK_SET);
...
static NV_INT32 recover_pfm_file (NV_CHAR *path)
{
    ...
    NV_INT32       i, j, list_size, line_size, num_bytes, percent, bp_type = 0, list_version = 0, 
                   old_percent, bin_hnd, index_hnd, hnd;
    NV_FLOAT32     temp;
    NV_CHAR        chk_file[512], line_path[512], bin_path[512],
                   index_path[512], string[1024], hdr[BIN_HEADER_SIZE];
    ...
    if ((bin_hnd = hfopen (bin_path, "rb+")) == -1) return (-1);

    hfwrite (hdr, BIN_HEADER_SIZE, 1, bin_hnd);
    ...
    if ((index_hnd = hfopen (index_path, "rb+")) == -1) return (-1);

    hftruncate (index_hnd, index_size);
...
NV_INT32 open_pfm_file (PFM_OPEN_ARGS *open_args)
{
    ...
    {
        status = create_list_file (hnd, dir_list_path,
    ...
    /*  Check for the pfm_double_bit_pack type for pre-existing PFMs.  The change occurred at version 4.5 for
        ...
        We're using that to determine who built the PFM so we can read it properly.  */
    ...
    if (new)
      ...
      {
        pfm_bp_type[hnd] = 0;
        if (strcmp (open_args->head.average_name, "Average Depth") && list_file_ver[hnd] >= 45) pfm_bp_type[hnd] = 1;
      }
...
NV_INT32 read_bin_row (NV_INT32 hnd, NV_INT32 length, NV_INT32 row, NV_INT32 column, BIN_RECORD a[])
{
    ...
    hfseek (bin_handle[hnd], address, SEEK_SET);
    hfread (buffer, size, 1, bin_handle[hnd]);
...
NV_INT32 read_bin_record_validity_index (NV_INT32 hnd, NV_I32_COORD2 coord, NV_U_INT32 *validity)
{
    ...
    if (!(hfread (bin_record_data[hnd], bin_off[hnd].record_size, 1,
        bin_handle[hnd])))
    {
...
NV_INT32 read_cov_map_index (NV_INT32 hnd, NV_I32_COORD2 coord, NV_U_BYTE *cov)
{
    ...
    else
    {
        ...
        if (hnd != prev_hnd || coord.y != prev_row)
        {
          ...
          hfread (row, 1, bin_header[hnd].bin_width, bin_handle[hnd]);
        }
...
NV_INT32 write_cov_map_index (NV_INT32 hnd, NV_I32_COORD2 coord, NV_U_BYTE cov)
{
    ...
    hfseek (bin_handle[hnd], address, SEEK_SET);
    hfwrite (&cov, 1, 1, bin_handle[hnd]);
...
static NV_INT32 read_depth_record (NV_INT32 hnd, DEPTH_RECORD *depth, NV_INT32 *count)
{
    ...
    if (bin_record[hnd].num_soundings == *count || depth_record_modified[hnd])
    {
        ...
        hfseek (index_handle[hnd], bin_record_head_pointer[hnd], SEEK_SET);

        if (!hfread (depth_record_data[hnd], dep_off[hnd].record_size, 1, index_handle[hnd]))
        {
    ...
    else
    {
        ...
        if (!((bin_record[hnd].num_soundings - *count) % hd[hnd].record_length))
        {
            ...
            hfseek (index_handle[hnd], continuation_pointer[hnd], SEEK_SET);
...
static NV_INT32 update_depth_record (NV_INT32 hnd, DEPTH_RECORD *depth, NV_BOOL set_modified)
{
    ...
    if (depth_record_address[hnd] != previous_depth_block[hnd])
    {
        ...
        hfseek (index_handle[hnd], depth_record_address[hnd], SEEK_SET);
...
static NV_INT32 add_depth_record (NV_INT32 hnd, DEPTH_RECORD *depth)
{
    ...
    if (bin_record[hnd].num_soundings)
    {
        ...
        */
        ...
        if (bin_record_tail_pointer[hnd] != depth_record_address[hnd] || !record_pos || *(depth_record_data[hnd]) == 0x0)
        {
            ...
            hfseek (index_handle[hnd], bin_record_tail_pointer[hnd], SEEK_SET);


            /*  Read the 'physical' depth record.  */

            hfread (depth_record_data[hnd], dep_off[hnd].record_size, 1, index_handle[hnd]);
        }
        ...
        if (!record_pos)
        {
            ...
            continuation_pointer[hnd] = hftell (index_handle[hnd]);
    ...
    else
    {
        ...
        bin_record_head_pointer[hnd] = hftell (index_handle[hnd]);
...
static NV_INT32 change_depth_record_avec (NV_INT32 hnd, DEPTH_RECORD *depth, NV_BOOL mod)
{
    ...
    if (depth_record_address[hnd] != previous_depth_block[hnd])
    {
        ...
        if (!hfread (depth_record_data[hnd], dep_off[hnd].record_size, 1,
            index_handle[hnd]))
        {
            sprintf (pfm_err_str, "Error reading depth record");
            return (pfm_error = UPDATE_DEPTH_RECORD_READ_DEPTH_RECORD_ERROR);
