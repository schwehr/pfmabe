...
/*!
   ...
   by IVS.  They are required for pre-4.5 PFM structures that have more than
   one "hugefile".  I'm not too sure about the inconsistent part but I was
   using only 31 of the available 32 low-order bits in order to avoid sign
   issues with the low order 32 bits (since I was using math to cut and 
   paste).  Unfortunately, IVS decided to use 32 bits and bit operations
   (I'll admit, a much more elegant solution) which broke backward (and
   cross) compatibility with previous PFM structures that had multiple
   "hugefiles".   JCD <br><br>

   Function        pre_6_double_bit_pack - Old version of pfm_double_bit_pack.

****************************************************************************/ 

void pre_6_double_bit_pack (NV_U_BYTE buffer[], NV_U_INT32 start, NV_U_INT32 numbits, NV_INT64 value) 
{
  NV_INT32            high_order, low_order;
...
/*! 
   ...
   Function        pre_6_double_bit_unpack - Old version of pfm_double_bit_unpack.

****************************************************************************/ 

NV_U_INT64 pre_6_double_bit_unpack (NV_U_BYTE buffer[], NV_U_INT32 start, NV_U_INT32 numbits) 
{
  NV_U_INT32          high_order, low_order;
  ...
  return ((NV_U_INT64) high_order * NV_INT32_MAX + (NV_INT64) low_order);
}
