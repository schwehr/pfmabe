...
NV_INT32 write_cached_bin_record (NV_INT32 hnd, BIN_RECORD_SUMMARY *bin_summary )
{
    ...
    PFM_DBL_BIT_PACK (bin_record_data[hnd], bin_off[hnd].head_pointer_pos, hd[hnd].record_pointer_bits, bin_record_head_pointer[hnd] );

    PFM_DBL_BIT_PACK (bin_record_data[hnd], bin_off[hnd].tail_pointer_pos, hd[hnd].record_pointer_bits, bin_record_tail_pointer[hnd] );
...
static NV_INT32 write_cached_depth_buffer (NV_INT32 hnd, NV_U_BYTE *buffer, NV_INT64 address)
{
    ...
    if (!(PFM_FWRITE (buffer, dep_off[hnd].record_size, 1, index_handle[hnd])))
    {
...
static void prepare_cached_depth_buffer (NV_INT32 hnd, DEPTH_SUMMARY *depth, NV_U_BYTE *depth_buffer,
                                         NV_BOOL preallocateBuffer)
{
    ...
    if (depth->continuation_pointer == 0)
    {
        ...
        if ( depth->chain.tail == -1 )
        {
            ...
            PFM_FSEEK (index_handle[hnd], 0LL, SEEK_END);

            /*  Set the head & tail pointers for the bin record.  */

            depth->chain.head = PFM_FTELL (index_handle[hnd]);
    ...
    PFM_DBL_BIT_PACK (depth_buffer, dep_off[hnd].continuation_pointer_pos, hd[hnd].record_pointer_bits, depth->continuation_pointer);
...
static NV_INT32 write_cached_depth_summary (NV_INT32 hnd, BIN_RECORD_SUMMARY *bin_summary)
{
    NV_INT32 status;
    DEPTH_LIST *buffer;
    ...
    if (bin_summary->num_soundings > 0) {
        ...
        if ( bin_summary->depth.previous_chain != -1 )
        {
            ...
            {
                PFM_FSEEK (index_handle[hnd], 0LL, SEEK_END);
                bin_summary->depth.continuation_pointer = PFM_FTELL( index_handle[hnd] );
    ...
    if (bin_summary->num_soundings != 0)
    {
        ...
        */
        if( bin_summary->depth.last_depth_buffer == 0x0 ) 
        {
            ...
            PFM_FSEEK (index_handle[hnd], bin_summary->depth.chain.tail, SEEK_SET);
            ...
            PFM_FREAD (bin_summary->depth.buffers.depths, dep_off[hnd].record_size, 1, index_handle[hnd]);
...
static NV_INT32 flush_depth_cache (NV_INT32 hnd)
{
    NV_INT32  r, c;
    NV_INT32  status;
...
NV_INT32 read_cached_depth_records (NV_INT32 hnd, BIN_RECORD_SUMMARY **bin_summary)
{
    ...
    if ((*bin_summary)->num_soundings > 0)
    {
        ...
        PFM_FSEEK (index_handle[hnd], (*bin_summary)->depth.chain.head, SEEK_SET);
        (*bin_summary)->depth.previous_chain = (*bin_summary)->depth.chain.head;

        /*  Read the 'physical' depth record.  */

        PFM_FREAD (buffer->depths, dep_off[hnd].record_size, 1, index_handle[hnd]);
#if 0
       if ((*bin_summary)->num_soundings > 6) {
        ...
        {
            PFM_FSEEK (index_handle[hnd], (*bin_summary)->depth.continuation_pointer, SEEK_SET);
            ...
            fflush(stdout);
#endif
            (*bin_summary)->depth.continuation_pointer = PFM_DBL_BIT_UNPACK (buffer->depths, dep_off[hnd].continuation_pointer_pos,
                                                                             hd[hnd].record_pointer_bits);
        }
    }
    return 1;
