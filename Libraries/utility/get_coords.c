...
/*!
  ...
  - Module Name:        get_coords

  - Programmer:         Jan C. Depner

  - Date Written:       September 1992

  - Purpose:            Gets a pair of geographic coordinates from the
                        user.
...
NV_INT32 get_coords (NV_CHAR *lat_hemi, NV_INT32 *lat_deg, NV_INT32 *lat_min, NV_FLOAT32 *lat_sec,
NV_CHAR *lon_hemi, NV_INT32 *lon_deg, NV_INT32 *lon_min, NV_FLOAT32 *lon_sec)
{
    NV_INT32         i, j, sign;
    ...
    do
    {
      ngets (string, sizeof (string), stdin);
        ...
        if (strstr (string, "done") != NULL) return (1);

        if (strstr (string, ",") == NULL)
            printf (" Missing comma in input, please re-enter position\n");
    ...
    for (j = 0 ; j < 2 ; j++)
    {
        ...
        if (j)
        {
            ...
            *lon_sec = (NV_FLOAT32)((fmin - *lon_min) * 60.0 + 0.00001);
        }
        else
        {
            if (sign)
            ...
            *lat_deg = (NV_INT32)fdeg;
            fmin = (NV_INT32)((fdeg - *lat_deg) * 60.0);
            *lat_min = (NV_INT32)(fmin + 0.00001);
            *lat_sec = (NV_FLOAT32)((fmin - *lat_min) * 60.0 + 0.00001);
        }
    }
