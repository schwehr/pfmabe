  ...
  for (i = 0 ; i < CZMIL_MAX_FILES ; i++)
    {
      if (czmilh[i].cxy_fp != NULL)
        {
          if (czmilh[i].cxy_created)
            {
              /*  If we were in the process of creating a file we need to remove it since it isn't finished.  */

              fclose (czmilh[i].cxy_fp);
              remove (czmilh[i].cxy_path);


              /*  Same with the CDX file but we're not implementing this for now.  

              fclose (czmilh[i].cdx_fp);
              remove (czmilh[i].cdx_path);  */


              /*  If we had a CWF file we need to get rid of that as well.  */

              if (czmilh[i].cwf_fp != NULL)
                {
                  fclose (czmilh[i].cwf_fp);
                  remove (czmilh[i].cwf_path);
                }
            }
        }
    }
...
static void czmil_get_string (const NV_CHAR *in, NV_CHAR *out)
{
...
/*!
 ...
 - Purpose:            Generic replacement for POSIX basename.  One advantage to this routine
                       over the POSIX one is that it doesn't destroy the input path.  This
                       works on Windoze even when using MSYS (both types of specifiers).
...
static NV_CHAR *czmil_gen_basename (const NV_CHAR *path)
{
  static NV_CHAR  basename[1024];
...
static void czmil_bit_pack (NV_U_BYTE buffer[], NV_U_INT32 start, NV_U_INT32 numbits, NV_INT32 value) 
{ 
...
static NV_U_INT32 czmil_bit_unpack (NV_U_BYTE buffer[], NV_U_INT32 start, NV_U_INT32 numbits) 
{ 
  NV_INT32                start_byte, end_byte, start_bit, end_bit, i; 
  NV_U_INT32              value; 
...
/*!
  ...
  - Function        double_bit_pack - Packs a long long integer value into consecutive
                    bits in buffer.
...
static void czmil_double_bit_pack (NV_U_BYTE buffer[], NV_U_INT32 start, NV_U_INT32 numbits, NV_INT64 value) 
{
  ...
  if (numbits < 33)
    {
      czmil_bit_pack (buffer, start, numbits, low_order);
    }
  else
    {
...
/*!
  ...
  - Function        double_bit_unpack - Unpacks a long long integer value from consecutive
                    bits in buffer.
...
static NV_U_INT64 czmil_double_bit_unpack (NV_U_BYTE buffer[], NV_U_INT32 start, NV_U_INT32 numbits) 
{
  NV_U_INT64          result;
  ...
  if (numbits < 33)
    {
      high_order = 0;
      low_order  = czmil_bit_unpack (buffer, start, numbits);
    }
  else
    {
...
static NV_CHAR *czmil_ngets (NV_CHAR *s, NV_INT32 size, FILE *stream)
{
