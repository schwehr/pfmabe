...
static NV_BOOL           no_file = NVFalse, first = NVTrue, restricted_data_read = NVFalse;
...
/***************************************************************************\
*                                                                           *
*   Module Name:        check_srtm2_topo                                    *
*                                                                           *
*   Programmer(s):      Jan C. Depner                                       *
*                                                                           *
*   Date Written:       November 2007                                       *
*                                                                           *
*   Purpose:            Checks for availability of the limited distribution *
*                       SRTM2 topo data file.                               *
*                                                                           *
*   Arguments:          None                                                *
*                                                                           *
...
NV_BOOL check_srtm2_topo ()
{
  ...
  sprintf (file, "%s%1csrtm2%1csrtm2_block_map.dat", dir, SEPARATOR, SEPARATOR);
...
/***************************************************************************\
*                                                                           *
*   Module Name:        check_srtm2_restricted_data_read                    *
*                                                                           *
*   Programmer(s):      Jan C. Depner                                       *
*                                                                           *
*   Date Written:       November 2007                                       *
*                                                                           *
*   Purpose:            Returns NVTrue if we've actually read SRTM2         *
*                       restricted data.  We may have used all US 1 second  *
*                       unrestricted data.  This way we can check after the *
*                       fact.                                               *
*                                                                           *
*   Arguments:          None                                                *
*                                                                           *
...
/***************************************************************************\
*                                                                           *
*   Module Name:        read_srtm2_topo_one_degree                          *
*                                                                           *
*   Programmer(s):      Jan C. Depner                                       *
*                                                                           *
*   Date Written:       November 2007                                       *
...
*                       same format as the srtm2 data files.  The           *
...
*                       array           -   topo array                      *
*                                                                           *
*   Returns:            0 for all water cell, 2 for undefined cell,         *
*                       3600 (width/height of array if it's square), 1800   *
*                       (width of the array, the height will be 3600).      *
*                                                                           *
...
*                       if (lon < 0) lon--;                                 *
*                                                                           *
*                       wsize = read_srtm2_topo_one_degree (lat, lon,       *
*                           &array);                                        *
*                                                                           *
*                       if (wsize != 32767 && wsize > 2)                    *
*                         {                                                 *
*                           hsize = wsize;                                  *
...
*                                                                           *
*                                                                           *
*                       You should also call cleanup_srtm2_topo after you   *
...
NV_INT32 read_srtm2_topo_one_degree (NV_INT32 lat, NV_INT32 lon, NV_INT16 **array)
{
  ...
  NV_INT32               i, j, shift_lat, shift_lon, resolution, pos, wsize = 0, hsize = 0, status, ndx, vacc, block;
  NV_U_INT32             mpos;
  uLong                  csize;
  uLongf                 bsize;
  NV_INT64               address;
  ...
  if (block_map[block] != prev_block)
    {
      ...
      prev_block = block;
      sprintf (file, "%s%1csrtm2%1csrtm2_block_%03d.cte", dir, SEPARATOR, SEPARATOR, block_map[block] + 200);
  ...
  if (prev_lat != shift_lat || prev_lon != shift_lon)
    {
      ...
      address = srtm_double_bit_unpack (map, mpos, 36);
      mpos += 36;
      vacc = srtm_bit_unpack (map, mpos, 8);
      ...
      if (bit_box == NULL)
        {
          perror ("Allocating bit_box memory in read_srtm2_topo");
          exit (-1);
        }
      ...
      box = (NV_INT16 *) calloc (wsize * hsize, sizeof (NV_INT16));
      if (box == NULL)
        {
          perror ("Allocating box memory in read_srtm2_topo");
          exit (-1);
        }
              ...
              for (j = 0 ; j < wsize ; j++)
                {
                  ...
                  if (temp < null_val)
                    {
                      box[i * wsize + j] = last_val + temp - bias;
                      last_val = box[i * wsize + j];
                    }
                  else
                    {
                      box[i * wsize + j] = -32768;
...
NV_INT16 read_srtm2_topo (NV_FLOAT64 lat, NV_FLOAT64 lon)
{
  ...
  static NV_FLOAT64  hinc = 0.0, winc = 0.0;
...
/***************************************************************************\
*                                                                           *
*   Module Name:        cleanup_srtm2_topo                                  *
*                                                                           *
*   Programmer(s):      Jan C. Depner                                       *
*                                                                           *
*   Date Written:       November 2007                                       *
*                                                                           *
*   Purpose:            Closes the open srtm2 topo file, frees memory, and  *
