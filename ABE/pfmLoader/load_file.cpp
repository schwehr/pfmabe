 ...
 *
 ********************************************************************/
static void GSF_PFM_Processing (gsfRecords * gsf_records, PFM_LOAD_PARAMETERS load_parms[], PFM_DEFINITION pfm_def[], NV_INT32 percent)
{
  NV_INT32            i;
  NV_U_INT32          flags;
      ...
      if (gsfRead (gsf_handle, GSF_RECORD_SWATH_BATHYMETRY_PING, &gsf_data_id, &gsf_records, NULL, 0) < 0) 
      ...
      else
        {
          ...
          if (!nom_warning && load_parms[0].flags.nom && gsf_records.mb_ping.nominal_depth == NULL)
            {
              fprintf (stderr, "\nFor file %s\n", path);
              fprintf (stderr, "nominal depth requested, true depth used.\n");
              nom_warning = NVTrue;
            }
  ...
  if (hawkeye_contents_header->data_type != HAWKEYE_HYDRO_BIN_DATA)
    {
      if (!hawkeye_contents_header->available.Timestamp || !hawkeye_contents_header->available.Point_Latitude ||
        ...
        {
          fprintf (stderr, "Missing required point record fields in HAWKEYE file %s\n", path);
  ...
  if ((bagErr = bagFileOpen (&bagHnd, BAG_OPEN_READONLY, (u8 *) path)) != BAG_SUCCESS)
    {
      ...
      if (bagGetErrorString (bagErr, &errstr) == BAG_SUCCESS) fprintf (stderr, "%s: %s\n", path, errstr);
