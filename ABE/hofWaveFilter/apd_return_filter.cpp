...
NV_BOOL apd_return_filter (NV_INT32 rec __attribute__ ((unused)), NV_INT32 sub_rec, HYDRO_OUTPUT_T *hof_record, NV_INT32 apd_run_req,
                           NV_FLOAT32 slope_req, NV_INT32 ac_zero_offset, NV_INT32 ac_off_req, WAVE_DATA_T *wave_rec)
{
  ...
  //  Check the AC zero offset.

  if (wave_rec->apd[bin] - ac_zero_offset < ac_off_req) return (NVTrue);
  ...
  //  Loop through the APD data looking for the first drop (from the surface).  Skip the first 20
  //  bins so that we don't start looking in the noisy section prior to the surface return.

  for (NV_INT32 i = 20 ; i < HWF_APD_SIZE ; i++)
    {
      //  If we get three zeros in a row we want to reset the drop counter.

      NV_INT32 change = (wave_rec->apd[i] - wave_rec->apd[i - 1]) + (wave_rec->apd[i - 1] - wave_rec->apd[i - 2]) +
        (wave_rec->apd[i - 2] - wave_rec->apd[i - 3]);

      if (!change) drop = 0;


      if (wave_rec->apd[i] - wave_rec->apd[i - 1] <= 0)
        {
  ...
  NV_INT32 length = qMin (bin + 50, HWF_APD_SIZE - 1);
  ...
  run = peak - start_data;
  slope = (NV_FLOAT32) (wave_rec->apd[peak] - wave_rec->apd[start_data]) / (NV_FLOAT32) run;


  length = qMin (peak + 50, HWF_APD_SIZE - 1);
  ...
  for (NV_INT32 i = peak ; i < length ; i++)
    {
      if (wave_rec->apd[i] - wave_rec->apd[i - 1] > 1)
        {
          end_data = i;
          break;
        }
  ...
  if (back_run < run)
    ...
    {
      backslope = (NV_FLOAT32) (wave_rec->apd[peak] - wave_rec->apd[end_data]) / (NV_FLOAT32) back_run;
    }


  //  If the slope is less than "slope_req" or the backslope is less than "slope_req" or the run length
  //  is less than "apd_run_req" we want to kill the return.

  if (run < apd_run_req || slope < slope_req)// || backslope < slope_req)
    {
      //fprintf(stderr,"%s %d %d %d %d %d\n",__FILE__,__LINE__,bin,start_data, peak, end_data);
      //fprintf(stderr,"%s %d %d %d %f %f %f\n",__FILE__,__LINE__,run,apd_run_req,slope, backslope,slope_req);
      return (NVTrue);
    }
