...
pfmLoad::pfmLoad (QWidget *parent, QString parm_file)
  : QWizard (parent)
{
  ...
  //  If a parameter file was read, populate the inputPage file list with the input files read from the 
  //  parameter file.
...
void pfmLoad::initializePage (int id)
{
  ...
  //  PFM pages.
  ...
  else if (id >= 1 && id < MAX_LOAD_FILES + 2)
    {
      if (id == 1)
        {
          pfm_global.cache_mem = NINT (field ("mem").toDouble ());
...
pfmLoad::slotCustomButtonClicked (int id)
{
          ...
          if ((pfm_def[i].hnd = open_existing_pfm_file (&pfm_def[i].open_args)) < 0)
            {
              ...
              QMessageBox::warning (this, tr ("pfmLoad"), tmp);
  ...
  switch (id)
    ...
    case QWizard::CustomButton1:
      ...
      sprintf (error_file, "pfmLoad_error_file_%d.tmp", getpid ());
...
#warning Uncomment the creation_software setting when we go to version 6.0 of the PFM library!
          ...
          if (input_file_def[i].status)
            {
              ...
              switch (input_file_def[i].type)
                {
                case PFM_GSF_DATA:
                  out_of_range[i] += load_gsf_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;

                case PFM_WLF_DATA:
                  out_of_range[i] += load_wlf_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;

                case PFM_HAWKEYE_HYDRO_DATA:
                case PFM_HAWKEYE_TOPO_DATA:
                  out_of_range[i] += load_hawkeye_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;

                case PFM_CHARTS_HOF_DATA:
                  out_of_range[i] += load_hof_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;

                case PFM_SHOALS_TOF_DATA:
                  out_of_range[i] += load_tof_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;

                case PFM_UNISIPS_DEPTH_DATA:
                  out_of_range[i] += load_unisips_depth_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;

                case PFM_NAVO_ASCII_DATA:
                  out_of_range[i] += load_hypack_xyz_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;

                case PFM_NAVO_LLZ_DATA:
                  out_of_range[i] += load_llz_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;

                case PFM_CZMIL_DATA:
                  out_of_range[i] += load_czmil_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;

                case PFM_BAG_DATA:
                  out_of_range[i] += load_bag_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;

                case PFM_ASCXYZ_DATA:
                  out_of_range[i] += load_ivs_xyz_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;

                case PFM_DTED_DATA:
                  out_of_range[i] += load_dted_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;

                case PFM_CHRTR_DATA:
                  out_of_range[i] += load_chrtr_file (pfm_file_count, count, input_file_def[i].name, progress.fbar, errfp, load_parms, pfm_def);
                  break;
                }
          ...
          if ((pfm_def[k].hnd = open_existing_pfm_file (&pfm_def[k].open_args)) < 0)
            {
              ...
              QMessageBox::warning (this, tr ("pfmLoad recompute"), tmp);
    ...
    case QWizard::CustomButton2:
      QFileDialog *fd = new QFileDialog (this, tr ("pfmLoad save parameter file"));
      ...
      fd->selectFilter (tr ("pfmLoad parameter file (*.prm)"));
...
void 
pfmLoad::slotCubeReadyReadStandardError ()
{
...
void 
pfmLoad::slotCubeReadyReadStandardOutput ()
{
  static QString resp_string = "";
...
void 
pfmLoad::slotCubeError (QProcess::ProcessError error)
{
  switch (error)
    {
    case QProcess::FailedToStart:
      QMessageBox::critical (this, tr ("pfmLoad CUBE"), tr ("Unable to start the CUBE process!"));
      break;

    case QProcess::Crashed:
      QMessageBox::critical (this, tr ("pfmLoad CUBE"), tr ("The CUBE process crashed!"));
      break;

    case QProcess::Timedout:
      QMessageBox::critical (this, tr ("pfmLoad CUBE"), tr ("The CUBE process timed out!"));
      break;

    case QProcess::WriteError:
      QMessageBox::critical (this, tr ("pfmLoad CUBE"), tr ("There was a write error from the CUBE process!"));
      break;

    case QProcess::ReadError:
      QMessageBox::critical (this, tr ("pfmLoad CUBE"), tr ("There was a read error from the CUBE process!"));
      break;

    case QProcess::UnknownError:
      QMessageBox::critical (this, tr ("pfmLoad CUBE"), tr ("The CUBE process died with an unknown error!"));
      break;
    }
}
...
void 
pfmLoad::slotCubeDone (int exitCode __attribute__ ((unused)), QProcess::ExitStatus exitStatus __attribute__ ((unused)))
{
