...
QImage *geotiff (NV_F64_XYMBR mbr, NV_CHAR *geotiff_file, NV_F64_XYMBR *geotiff_mbr)
{
  ...
  if (poDataset != NULL)
          ...
          if (mbr.max_y >= GeoTIFF_slat && mbr.min_y <= GeoTIFF_nlat && mbr.max_x >= GeoTIFF_wlon && mbr.min_x <= GeoTIFF_elon)
            {
              ...
              if (full_res_image == NULL || full_res_image->width () == 0 || full_res_image->height () == 0)
                {
                  QMessageBox::critical (0, "bagViewer", bagViewer::tr ("Unable to open or read GeoTIFF file!"));
                  delete poDataset;
                  return (NULL);
                }
              */
              ...
              if (rasterCount < 3)
                {
                  delete poDataset;
                  QMessageBox::critical (0, "bagViewer", bagViewer::tr ("Not enough raster bands in geoTIFF"));
                  return (NULL);
                }
              ...
              for (NV_INT32 i = 0 ; i < rasterCount ; i++)
                {
                  ...
                  if (dataType[i] != "Byte")
                    {
                      delete poDataset;
                      QMessageBox::critical (0, "bagViewer", bagViewer::tr ("Cannot handle %1 data type").arg (dataType[i]));
                      return (NULL);
                    }
              ...
              if (full_res_image == NULL || full_res_image->width () == 0 || full_res_image->height () == 0)
                {
                  QMessageBox::critical (0, "bagViewer", bagViewer::tr ("Unable to open image!"));
                  delete poDataset;
                  return (NULL);
                }
              ...
              if (!w || !h || !d)
                {
                  QMessageBox::critical (0, "bagViewer", bagViewer::tr ("The image is too large for OpenGL (%1 X %2)").arg (wide).arg (high));
    ...
    {
      QMessageBox::warning (0, "bagViewer", bagViewer::tr ("Unable to read tiff file."));
    }
