...
externalFilter::externalFilter (QWidget *parent, nvMap *ma, OPTIONS *op, MISC *mi, NV_INT32 prog, NV_BOOL *failed):
  QProcess (parent)
{
...
externalFilter::slotError (QProcess::ProcessError error)
{
  ...
  switch (error)
    ...
    case QProcess::FailedToStart:
      QMessageBox::critical (pa, tr ("pfmEdit area program"), tr ("Unable to start the area program!") + "\n" + commandLine);
      break;

    case QProcess::Crashed:
      QMessageBox::critical (pa, tr ("pfmEdit area program"), tr ("The area program crashed!") + "\n" + commandLine);
      break;

    case QProcess::Timedout:
      QMessageBox::critical (pa, tr ("pfmEdit area program"), tr ("The area program timed out!") + "\n" + commandLine);
      break;

    case QProcess::WriteError:
      QMessageBox::critical (pa, tr ("pfmEdit area program"), tr ("There was a write error to the area program!") + "\n" + commandLine);
      break;

    case QProcess::ReadError:
      QMessageBox::critical (pa, tr ("pfmEdit area program"), tr ("There was a read error from the area program!") + "\n" + commandLine);
      break;

    case QProcess::UnknownError:
      QMessageBox::critical (pa, tr ("pfmEdit area program"), tr ("The area program died with an unknown error!") + "\n" + commandLine);
      break;
    }
