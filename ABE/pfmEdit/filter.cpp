...
NV_BOOL InitializeAreaFilter (NV_INT32 width, NV_INT32 height, MISC *misc)
{
  ...
  if (bin_depth == NULL)
    {
      QMessageBox::critical (0, pfmEdit::tr ("pfmEdit filter"), pfmEdit::tr ("Unable to allocate memory for filtering!"));
      return (NVFalse);
    }
...
NV_BOOL AreaFilter (nvMap *map, OPTIONS *options, MISC *misc, NV_FLOAT64 bin_diagonal, NV_INT32 row, NV_INT32 col,
                    NV_FLOAT64 *mx, NV_FLOAT64 *my, NV_INT32 poly_count)
{
                  ...
                  if (inside_polygon2 (mx, my, poly_count, misc->data[bin_depth[row][col].rv[i].rec].x, misc->data[bin_depth[row][col].rv[i].rec].y))
                    {
                      ...
                      if (!check_bounds (map, options, misc, misc->data[bin_depth[row][col].rv[i].rec].x, misc->data[bin_depth[row][col].rv[i].rec].y, 
                          ...
                          if (feature_count)
                            {
                              for (NV_INT32 j = 0 ; j < feature_count ; j++)
                                {
                                  NV_FLOAT64 dist;
                                  pfm_geo_distance (misc->pfm_handle[0], feature[j].lat, feature[j].lon, misc->data[bin_depth[row][col].rv[i].rec].y, 
...
NV_BOOL filterPolyArea (nvMap *map, OPTIONS *options, MISC *misc, NV_FLOAT64 *mx, NV_FLOAT64 *my, NV_INT32 count)
{
  ...
  if (misc->abe_share->open_args[0].head.mbr.min_y <= 0.0)
    {
      invgp (NV_A0, NV_B0, misc->abe_share->open_args[0].head.mbr.max_y, misc->abe_share->open_args[0].head.mbr.min_x, 
             misc->abe_share->open_args[0].head.mbr.max_y - (misc->abe_share->open_args[0].head.y_bin_size_degrees), 
             misc->abe_share->open_args[0].head.mbr.min_x + (misc->abe_share->open_args[0].head.x_bin_size_degrees), &bin_diagonal, &az);
    }
  else
    {
      invgp (NV_A0, NV_B0, misc->abe_share->open_args[0].head.mbr.min_y, misc->abe_share->open_args[0].head.mbr.min_x, 
             misc->abe_share->open_args[0].head.mbr.min_y + (misc->abe_share->open_args[0].head.y_bin_size_degrees), 
             misc->abe_share->open_args[0].head.mbr.min_x + (misc->abe_share->open_args[0].head.x_bin_size_degrees), &bin_diagonal, &az);
    }
  ...
  misc->statusProgLabel->setText (pfmEdit::tr ("Filtering..."));
  ...
  for (NV_INT32 i = 1 ; i < height - 1 ; i++)
    {
      ...
      for (NV_INT32 j = 1 ; j < width - 1 ; j++)
        {
          AreaFilter (map, options, misc, bin_diagonal, i, j, mx, my, count);
        }
