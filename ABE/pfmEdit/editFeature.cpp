  ...
  QDialog (pt, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  setWindowTitle (tr ("pfmEdit Edit Feature"));
...
editFeature::slotViewError (QProcess::ProcessError error)
{
  switch (error)
    ...
    case QProcess::FailedToStart:
      QMessageBox::critical (this, tr ("pfmEdit image viewer"), tr ("Unable to start the image viewer process!"));
      break;

    case QProcess::Crashed:
      QMessageBox::critical (this, tr ("pfmEdit image viewer"), tr ("The image viewer process crashed!"));
      break;

    case QProcess::Timedout:
      QMessageBox::critical (this, tr ("pfmEdit image viewer"), tr ("The image viewer process timed out!"));
      break;

    case QProcess::WriteError:
      QMessageBox::critical (this, tr ("pfmEdit image viewer"), 
...
//!  This is the stderr read return from the feature image viewer QProcess.  Hopefully you won't see anything here
...
editFeature::slotAssignSnippet ()
{
  ...
  QFileDialog *fd = new QFileDialog (this, tr ("pfmEdit Assign Image File"));
      ...
      if (!misc->bfd_open)
        {
          ...
          QFileDialog fd (parent, tr ("pfmEdit Assign Feature File"));
          fd.setViewMode (QFileDialog::List);
          fd.setDirectory (QFileInfo (misc->abe_share->open_args[0].list_path).dir ());
              ...
              if ((misc->bfd_handle = binaryFeatureData_open_file (filename, &misc->bfd_header, BFDATA_UPDATE)) < 0)
                  ...
                  if ((misc->bfd_handle = binaryFeatureData_create_file (misc->abe_share->open_args[0].target_path, misc->bfd_header)) < 0)
                    {
                      ...
                      QMessageBox::warning (parent, tr ("pfmEdit"), tr ("Unable to create feature file\nReason: ") + msg);
                      return;
                    }
              ...
              else
                {
                  ...
                  if ((misc->bfd_handle = binaryFeatureData_open_file (misc->abe_share->open_args[0].target_path, &misc->bfd_header, BFDATA_UPDATE)) < 0)
                    {
                      QString msg = QString (binaryFeatureData_strerror ());
                      QMessageBox::warning (parent, tr ("pfmEdit"), tr ("Unable to open feature file\nReason: ") + msg);
                      return;
                    }
...
editFeature::slotLidar ()
{
  ...
  lidarD->setWindowTitle (tr ("pfmEdit LIDAR Types"));
