...
#ifndef _PFM_EDIT_DEF_H_
#define _PFM_EDIT_DEF_H_
...
#define         SELECT_VIEW                 6   //!<  Make sure this doesn't run in to EAST_VIEW, SOUTH_VIEW, or PLAN_VIEW
#define         KEEP_POLYGON                7
#define         UNSET_SINGLE                8
#define         SET_MULTIPLE                9
#define         MOVE_FEATURE                10
#define         DELETE_FEATURE              11
#define         EDIT_FEATURE                12
#define         ADD_FEATURE                 13
#define         SHIFT_LINE                  14
#define         HOTKEY_POLYGON              15
#define         SET_REFERENCE               16
#define         UNSET_REFERENCE             17
#define         RESET_SINGLE                18
#define         MASK_INSIDE_RECTANGLE       19
#define         MASK_OUTSIDE_RECTANGLE      20
#define         MASK_INSIDE_POLYGON         21
#define         MASK_OUTSIDE_POLYGON        22
#define         MEASURE                     23
#define         POLY_FILTER_MASK            24
#define         RECT_FILTER_MASK            25
#define         AV_DISTANCE_TOOL            26
#define         HIGHLIGHT_POLYGON           27
#define         CLEAR_POLYGON               28


#define         NUMSHADES                   100
#define         FREEZE_POPUP                8
...
#define         FILTER_MASK                 997
#define         INVALID                     998
#define         HIGHLIGHTED                 999
#define         PLAN_VIEW                   0
#define         SOUTH_VIEW                  1
#define         EAST_VIEW                   2
#define         LINE_WIDTH                  2
...
/*!
  ...
  and in pfmEdit.cpp (look for the tool buttons that have one of these defined names attached).  If the
...
#define         RESET_KEY                   3
#define         REDRAW_KEY                  4
#define         TOGGLE_CONTOUR_KEY          5
#define         DELETE_POINT_MODE_KEY       6
#define         DELETE_RECTANGLE_MODE_KEY   7
#define         DELETE_POLYGON_MODE_KEY     8
#define         FILTER_KEY                  9
...
#define         COLOR_BY_DEPTH_ACTION_KEY   21
#define         COLOR_BY_LINE_ACTION_KEY    22
#define         AV_DISTANCE_THRESHOLD_KEY   23
...
/*!
  ...
  it needs to be done here, in set_defaults.cpp, and add or subtract buttons in pfmEdit.cpp.
*/
...
//!  The OPTIONS structure contains all those variables that can be saved to the users pfmEdit QSettings.
...
typedef struct
{
  ...
  NV_BOOL     display_grid;               //!<  Display semi-transparent PFM bin grid overlay
  ...
  NV_BOOL     auto_redraw;                //!<  Toggle auto redraw after edit
  ...
  NV_BOOL     alt_view_by_line;           //!<  alternate view color by line or distance
  NV_BOOL     plan_view_by_line;          //!<  plan view color by line or depth
  ...
  NV_INT32    GeoTIFF_alpha;              //!<  0 - invisible, 255 - opaque, range in between - some transparency
  NV_BOOL     display_GeoTIFF;            //!<  Set to display GeoTIFF
  ...
  NV_BOOL     kill_and_respawn;           /*!<  Set this to NVTrue to kill the ancillary programs on exit and 
                                                ...
                                                running and reconnect to pfmEdit when it restarts.  This is a blanket
  ...
  NV_BOOL     hk_poly_filter[NUMPROGS];   //!<  Whether the key can only be used for hotkey polygons (1) or external filter (2), otherwise 0
  NV_BOOL     hk_planview_only[NUMPROGS]; //!<  Whether the key can only be used in plan view
  NV_INT32    state[NUMPROGS];            /*!<  - Ancillary program state:
                                                    - 0 = program does not respawn on restart
                                                    - 1 = program respawns but was not up when pfmEdit last closed
...
typedef struct
{
  ...
  NV_INT32    save_view;                  //!<  Saved view if select view is discarded
  NV_INT32    save_mode;                  //!<  Saved active edit mode for exit (only delete functions)
  NV_BOOL     bounds_changed;             //!<  Section bounds changed flag
  ...
  NV_FLOAT64  *hotkey_poly_x;             //!<  hotkey polygon X points (allocated)
  NV_FLOAT64  *hotkey_poly_y;             //!<  hotkey polygon Y points (allocated)
  NV_FLOAT32  rotate_scale;               //!<  X scale of rotated view
  NV_INT32    rotate_offset;              //!<  X offset of rotated view
  NV_INT32    rotate_angle;               //!<  Rotation of view angle at 1/10 degree
  NV_FLOAT32  ortho_scale;                //!<  X scale of rotated view 
  NV_INT32    ortho_offset;               //!<  X offset of rotated view
  ...
  NV_BOOL     resized;                    //!<  resized flag used by the GeoTIFF drawing routine
  ...
  QColor      color_array[2][NUMSHADES];  //!<  Colors for points (0 is full color, 1 is semi-transparent)
  QColor      line_color_array[2][NUMSHADES * 2];  /*!<  Colors for points in color by line mode (0 is full color, 1 is semi-transparent,
                                                         first half is full value, second half is half value - darker)  */
  NV_INT32    last_drawn_index;           //!<  Last point drawn in case the user interrupts the drawing
  ...
  NV_INT32    map_border;                 //!<  Map border in pixels
  ...
  NV_INT32    view;                       //!<  Type of view (plan, south, east, pick)
  NV_FLOAT32  view_min;                   //!<  Minimum z value - 5% of range
  NV_FLOAT32  view_max;                   //!<  Maximum z value + 5% of range
  ...
  NV_FLOAT32  line_shift[MAX_ABE_PFMS][PFM_MAX_FILES]; //!<  Amount to shift lines (if any)
  NV_BOOL     shifted[MAX_ABE_PFMS];      //!<  Flag to indicate shifted lines
} MISC;


NV_INT32 check_bounds (nvMap *map, OPTIONS *options, MISC *misc, NV_INT32 index, NV_BOOL ck, NV_BOOL slice);
NV_INT32 check_bounds (nvMap *map, OPTIONS *options, MISC *misc, NV_FLOAT64 x, NV_FLOAT64 y, NV_FLOAT32 z,
                      NV_U_INT32 v, NV_BOOL mask, NV_INT16 pfm, NV_BOOL ck, NV_BOOL slice);
...
void set_area (nvMap *map, OPTIONS *options, MISC *misc, NV_FLOAT64 *x_bounds, NV_FLOAT64 *y_bounds,
               NV_FLOAT64 *z_bounds, NV_INT32 num_vertices, NV_INT32 rect_flag);
