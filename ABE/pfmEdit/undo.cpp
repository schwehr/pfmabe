          ...
          for (NV_INT32 i = 0 ; i < undo_levels - 1 ; i++)
            {
              ...
              if (misc->undo[i].val == NULL)
                {
                  QMessageBox::critical (0, pfmEdit::tr ("pfmEdit store undo"), pfmEdit::tr ("Unable to allocate UNDO validity memory!  Reason : %1").arg (strerror (errno)));
                  exit (-1);
                }
              ...
              if (misc->undo[i].num == NULL)
                {
                  QMessageBox::critical (0, pfmEdit::tr ("pfmEdit store undo"), pfmEdit::tr ("Unable to allocate UNDO num memory!  Reason : %1").arg (strerror (errno)));
                  exit (-1);
                }
...
NV_BOOL resize_undo (MISC *misc, OPTIONS *options, NV_INT32 undo_levels)
{
  ...
  if (undo_levels <= misc->undo_count)
    {
      QMessageBox::warning (0, pfmEdit::tr ("pfmEdit resize undo"), pfmEdit::tr ("Resizing to less than already existing undo blocks is not allowed!"));
      return (NVFalse);
    }
