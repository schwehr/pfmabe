...
NV_INT32 check_bounds (nvMap *map, OPTIONS *options, MISC *misc, NV_INT32 index, NV_BOOL ck, NV_BOOL slice)
{
  ...
  return (check_bounds (map, options, misc, x, y, z, v, m, pfm, ck, slice));
}
...
NV_INT32 check_bounds (nvMap *map, OPTIONS *options, MISC *misc, NV_FLOAT64 x, NV_FLOAT64 y, NV_FLOAT32 z, NV_U_INT32 v, NV_BOOL mask,
                       NV_INT16 pfm, NV_BOOL ck, NV_BOOL slice)
{
  NV_INT32         ix, iy, iz;
  ...
  //  Check for slicing if it is enabled.  In order to use semi-transparent values the return from the
  //  slice check must be changed to 1.

  if (slice)
    {
      scale_view (map, misc, x, y, 0.0, &ix, &iy, &iz);
      if (iz < misc->slice_min || iz > misc->slice_max) return (2);
    }
