...
//!  This paints the GeoTIFF under the point cloud (in plan view only).
  ...
  if (misc->GeoTIFF_init || fabs (prev_mbr.min_x - min_x) > 0.00000001 ||
      ...
      fabs (prev_mbr.max_y - max_y) > 0.00000001)
    {
      ...
      if (poDataset != NULL)
              ...
              if (max_y >= GeoTIFF_slat && min_y <= GeoTIFF_nlat && max_x >= GeoTIFF_wlon && min_x <= GeoTIFF_elon)
                {
                  ...
                  full_res_image = new QImage (misc->GeoTIFF_name);

                  if (options->GeoTIFF_alpha < 255)
                    {
                  ...
                  */
                  ...
                  if (rasterCount < 3)
                    {
                      delete poDataset;
                      QMessageBox::critical (0, "pfmEdit", pfmEdit::tr ("Not enough raster bands in geoTIFF"));
                      return;
                    }
                  ...
                  for (NV_INT32 i = 0 ; i < rasterCount ; i++)
                    {
                      ...
                      if (dataType[i] != "Byte")
                        {
                          delete poDataset;
                          QMessageBox::critical (0, "pfmEdit", pfmEdit::tr ("Cannot handle %1 data type").arg (dataType[i]));
                          return;
                        }
                  ...
                  if (full_res_image == NULL || full_res_image->width () == 0 || full_res_image->height () == 0)
                    {
                      QMessageBox::critical (0, "pfmEdit", pfmEdit::tr ("Unable to open image!"));
                      delete poDataset;
                      return;
                    }
        ...
        {
          QMessageBox::warning (map, pfmEdit::tr ("GeoTIFF"), pfmEdit::tr ("Unable to read tiff file."));
        }
    }
  else
    {
