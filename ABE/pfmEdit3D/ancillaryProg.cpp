...
ancillaryProg::ancillaryProg (QWidget *parent, nvMapGL *ma, OPTIONS *op, MISC *mi, NV_INT32 prog, NV_INT32 lock_point):
  QProcess (parent)
{
      ...
      if (appTimerCount > 10)
        {
          ...
          QMessageBox::information (pa, tr ("pfmEdit3D ancillary program"), tr ("The program ") + 
...
ancillaryProg::slotError (QProcess::ProcessError error)
{
  ...
  switch (error)
    ...
    case QProcess::FailedToStart:
      QMessageBox::critical (pa, tr ("pfmEdit3D ancillary program"), tr ("Unable to start the ancillary program!") + 
                             "\n" + commandLine);
      break;

    case QProcess::Crashed:
      QMessageBox::critical (pa, tr ("pfmEdit3D ancillary program"), tr ("The ancillary program crashed!") + 
                             "\n" + commandLine);
      break;

    case QProcess::Timedout:
      QMessageBox::critical (pa, tr ("pfmEdit3D ancillary program"), tr ("The ancillary program timed out!") + 
                             "\n" + commandLine);
      break;

    case QProcess::WriteError:
      QMessageBox::critical (pa, tr ("pfmEdit3D ancillary program"), 
...
ancillaryProg::slotDone (int exitCode __attribute__ ((unused)), QProcess::ExitStatus exitStatus __attribute__ ((unused)))
{
  ...
  //  The external program should set modcode to it's data type to signify that it has done something
          ...
          if (misc->abe_share->modified_point >= 0)
            {
              ...
              map->map_to_screen (misc->data[set_point].x, misc->data[set_point].y, misc->data[set_point].z, &x, &y, &z);

              //drawx (map, x, y, z, options->tracker_color);
            }

          //  This is a bit obscure - waveform monitor program(s) will look for this modcode to force a redraw.
