      ...
      if (row_width > 0)
        {
          ...
          if (row == NULL)
            {
              perror (pfmEdit3D::tr ("Allocating row in get_buffer").toAscii ());
              exit (-1);
            }
  ...
  if (!misc->dataShare->create (sizeof (POINT_CLOUD) * point_count, QSharedMemory::ReadWrite))
    {
      ...
      if (!misc->dataShare->create (sizeof (POINT_CLOUD) * point_count, QSharedMemory::ReadWrite))
        {
          QMessageBox::critical (0, pfmEdit3D::tr ("pfmEdit3D get_buffer"),
                                 pfmEdit3D::tr ("Unable to allocate %1 points (%2 bytes) of shared data memory.  Reason:%3").arg
                                 (point_count) .arg (sizeof (POINT_CLOUD) * point_count).arg
                                 (misc->dataShare->errorString ()));
          fprintf (stderr, "Unable to allocate %d points (%d bytes) of shared data memory in get_buffer.c.  Reason:%s", point_count,
                   sizeof (POINT_CLOUD) * point_count, strerror (errno));
          exit (-1);
        }
    }
          ...
          if (binaryFeatureData_read_all_short_features (misc->bfd_handle, &misc->feature) < 0)
            {
              ...
              QMessageBox::warning (0, pfmEdit3D::tr ("pfmEdit3D"), pfmEdit3D::tr ("Unable to read feature records\nReason: ") + msg);
