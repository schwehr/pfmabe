...
QColor GetAVShotColor (NV_INT32 index, MISC *misc, OPTIONS *options)
{
  ...
  //  Check for color by attribute
  ...
  else
    {
      ...
      if (!options->color_index)
        {
          c_index = NINT (numshades - (-misc->data[index].z - misc->avb.actualMin) / misc->avb.actualRange * numshades);
        }
  ...
  if (c_index < 0)
  ...
  else
    {
      if (options->color_index == 1)
        {
          memcpy (tmp, misc->line_color_array[0][c_index], 4);			
        }
      else
        {
          memcpy (tmp, misc->color_array[0][c_index], 4);			
        }
    }

  rgbColor = QColor::fromRgb ((NV_INT32)tmp[0], (NV_INT32)tmp[1], (NV_INT32)tmp[2], (NV_INT32)tmp[3]);	

  QColor color = rgbColor.convertTo (QColor::Hsv);
