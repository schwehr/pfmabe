...
void scribe (nvMap *map, OPTIONS *options, MISC *misc, NV_FLOAT32 *ar, NV_INT32 ncc, NV_INT32 nrr, NV_FLOAT64 xorig, NV_FLOAT64 yorig)
{
  ...
  NV_FLOAT64              dcontour_x[CONTOUR_POINTS], dcontour_y[CONTOUR_POINTS], dcontour_z[CONTOUR_POINTS], dx, dy, cell_diag_length,
  ...
  while (getContour (&level, &index_contour, &num_points, contour_x, contour_y))
    {
      ...
      for (i = 1 ; i < num_points ; i++)
        {
          map->drawLine (NINT (xyz_x[i - 1]), NINT (xyz_y[i - 1]), NINT (xyz_x[i]), NINT (xyz_y[i]), options->contour_color, width, NVFalse, Qt::SolidLine);
        }
