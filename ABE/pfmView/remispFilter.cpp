...
remispFilter::remispFilter (MISC *mi, OPTIONS *op, NV_FLOAT64 *x, NV_FLOAT64 *y, NV_INT32 pc)
{
  ...
  mbr.max_x = -999999.0;
  mbr.max_y = -999999.0;
  for (NV_INT32 i = 0 ; i < poly_count ; i++)
    {
  ...
  for (NV_INT32 pfm = 0 ; pfm < misc->abe_share->pfm_count ; pfm++)
      ...
      if (misc->abe_share->display_pfm[pfm] && misc->average_type[pfm])
	{
	  ...
	  progText = pfmView::tr (" Reading PFM data from ") + QFileInfo (QString (misc->abe_share->open_args[pfm].list_path)).fileName ().remove (".pfm") + " ";
			  ...
			  if (!read_depth_array_index (misc->pfm_handle[pfm], coord, &depth, &recnum))
			      ...
			      for (NV_INT32 k = 0 ; k < recnum ; k++)
				{
				  switch (misc->average_type[pfm])
				    ...
				    case 1:
				    case 4:
				      if ((!(depth[k].validity & (PFM_INVAL | PFM_DELETED | PFM_REFERENCE))) && fabs (depth[k].xyz.z - bin.min_filtered_depth) < EPS &&
                                          !inside_polygon2 (mx, my, poly_count, depth[k].xyz.x, depth[k].xyz.y))
					{
					  add_point (&xyz_array, depth[k].xyz, &out_count);
					  found = NVTrue;
				    ...
				    case 3:
				    case 6:
				      if ((!(depth[k].validity & (PFM_INVAL | PFM_DELETED | PFM_REFERENCE))) && fabs (depth[k].xyz.z - bin.max_filtered_depth) < EPS &&
                                          !inside_polygon2 (mx, my, poly_count, depth[k].xyz.x, depth[k].xyz.y))
					{
					  add_point (&xyz_array, depth[k].xyz, &out_count);
					  found = NVTrue;
  ...
  for (NV_INT32 pfm = 0 ; pfm < misc->abe_share->pfm_count ; pfm++)
      ...
      if (misc->abe_share->display_pfm[pfm] && misc->average_type[pfm])
	{
	  ...
	  //  Initialize the MISP engine.
          ...
          progText = pfmView::tr (" Loading data into MISP for ") +
	  ...
	  for (NV_INT32 i = 0 ; i < out_count ; i++)
	      ...
	      if (!(out_count % inc))
		{
          ...
          progText = pfmView::tr (" Computing MISP surface for ") +
            QFileInfo (QString (misc->abe_share->open_args[pfm].list_path)).fileName ().remove (".pfm") + " ";
                              ...
                              if (inside_polygon2 (mx, my, poly_count, xy.x, xy.y))
                                {
                                  ...
                                  if (!land_mask_flag || !(bin.validity & PFM_USER_05))
                                    {
                                          ...
                                          if (!read_depth_array_index (misc->pfm_handle[pfm], coord, &depth, &recnum))
                                            {
                                              ...
                                              NV_FLOAT64 min_diff = 999999999.0;
                                              NV_INT32 ndx = -1;
                                                          ...
                                                          if (k != ndx || fabs (depth[k].xyz.z - array[j]) > options->contour_filter_envelope)
                                                            {
                                                              ...
                                                              NV_INT32 rc = update_depth_record_index (misc->pfm_handle[pfm], &depth[k]);
                                                              if (rc != SUCCESS)
                                                                {
...
void 
remispFilter::add_point (NV_F64_COORD3 **xyz_array, NV_F64_COORD3 xyz, NV_INT32 *count)
{
  *xyz_array = (NV_F64_COORD3 *) realloc (*xyz_array, (*count + 1) * sizeof (NV_F64_COORD3));
  if (*xyz_array == NULL)
    {
      perror (pfmView::tr ("Allocating xyz_array in remispFilter.").toAscii ());
      exit (-1);
    }
