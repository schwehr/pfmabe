  ...
  QDialog (parent, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  if (attribute)
    {
      single_step = misc->displayed_area_attr_range / (NV_FLOAT64) (NUM_SCALE_LEVELS - 1);
    }
  else
    {
      single_step = misc->displayed_area_range / (NV_FLOAT64) (NUM_SCALE_LEVELS - 1);
    }


  setWindowTitle (tr ("pfmView ") + ctype + tr (" Scale Lock"));
