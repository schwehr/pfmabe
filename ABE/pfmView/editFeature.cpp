...
//!  This is the feature editing dialog.
  ...
  QDialog (pt, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  //  If we are creating a feature, not editing an existing one...
  ...
  setWindowTitle (tr ("pfmView Edit Feature"));
  ...
  chartScale->setWhatsThis (editChartScaleText);
...
void 
editFeature::slotChartScale (int value)
{
...
editFeature::slotViewError (QProcess::ProcessError error)
{
  switch (error)
    ...
    case QProcess::FailedToStart:
      QMessageBox::critical (this, tr ("pfmView image viewer"), tr ("Unable to start the image viewer process!"));
      break;

    case QProcess::Crashed:
      QMessageBox::critical (this, tr ("pfmView image viewer"), tr ("The image viewer process crashed!"));
      break;

    case QProcess::Timedout:
      QMessageBox::critical (this, tr ("pfmView image viewer"), tr ("The image viewer process timed out!"));
      break;

    case QProcess::WriteError:
      QMessageBox::critical (this, tr ("pfmView image viewer"), 
...
editFeature::slotAssignSnippet ()
{
  ...
  QFileDialog *fd = new QFileDialog (this, tr ("pfmView Assign Image File"));
      ...
      if (!misc->bfd_open)
        {
          ...
          QFileDialog fd (parent, tr ("pfmView Assign Feature File"));
          fd.setViewMode (QFileDialog::List);
              ...
              if ((misc->bfd_handle = binaryFeatureData_open_file (filename, &misc->bfd_header, BFDATA_UPDATE)) < 0)
                  ...
                  if ((misc->bfd_handle = binaryFeatureData_create_file (misc->abe_share->open_args[0].target_path, misc->bfd_header)) < 0)
                    {
                      ...
                      QMessageBox::warning (parent, tr ("pfmView"), tr ("Unable to create feature file\nReason: ") + msg);
                      return;
                    }
              ...
              else
                {
                  ...
                  if ((misc->bfd_handle = bfd_check_file (misc, misc->abe_share->open_args[0].target_path, &misc->bfd_header, BFDATA_UPDATE)) < 0)
                    {
                      QString msg = QString (binaryFeatureData_strerror ());
                      QMessageBox::warning (parent, tr ("pfmView"), tr ("Unable to open feature file\nReason: ") + msg);
                      return;
                    }
...
//!  Group features as children under a parent feature.
...
editFeature::slotGroup ()
{
  ...
  NV_FLOAT32 min_z = bfd_record.depth;
  NV_INT32 min_feature = -1;
  ...
  NV_BOOL shoalest = NVFalse;

  if (min_feature >= 0)
    {
      QMessageBox msgBox (this);
      msgBox.setIcon (QMessageBox::Question);
      msgBox.setText (tr ("pfmView"));
      msgBox.setInformativeText (tr ("Do you wish to use the current feature or the shoalest in the polygon as the master?"));
      msgBox.setStandardButtons (QMessageBox::Yes | QMessageBox::No | QMessageBox::Cancel);
      msgBox.setDefaultButton (QMessageBox::No);
      msgBox.setButtonText (QMessageBox::Yes, tr ("Current"));
      msgBox.setButtonText (QMessageBox::No, tr ("Shoalest"));
  ...
  //  If it's a new feature we need to save it and re-read the feature file.
  ...
  if (new_feature)
    {
      ...
      //  If we want to use the shoalest feature as the "master" we need to save the record but without the defined polygon.
      ...
      //  If we want to use the shoalest point we need to read it now that we have saved the new feature.

      if (shoalest)
        {
          binaryFeatureData_read_record (misc->bfd_handle, misc->feature[min_feature].record_number, &bfd_record);
    ...
    {
      //  If we want to use the shoalest point we need to save the current feature and read the shoalest in as
      //  the current feature.
...
//!  Ungroup children and turn them in to normal features.
...
editFeature::slotUngroup ()
{
  ...
  if (!bfd_record.child_record || bfd_record.parent_record)
    {
      QMessageBox::warning (this, tr ("pfmView"), tr ("This is not a master feature."));
      return;
    }
...
//!  The following allows you to assign a standard description to the features.

void 
editFeature::slotRocks ()
{
  rockD = new QDialog (this, (Qt::WindowFlags) Qt::WA_DeleteOnClose);
  rockD->setWindowTitle (tr ("pfmView Rock, Wreck, and Obstruction Types"));
...
editFeature::slotOffshore ()
{
  ...
  offshoreD->setWindowTitle (tr ("pfmView Offshore Installation Types"));
...
editFeature::slotLights ()
{
  ...
  lightD->setWindowTitle (tr ("pfmView Light, Buoy, and Beacon Types"));
...
editFeature::slotLidar ()
{
  ...
  lidarD->setWindowTitle (tr ("pfmView LIDAR Types"));
