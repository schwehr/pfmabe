  ...
  QWizardPage (parent)
{
  ...
  progress->mbox = new QGroupBox (tr ("Determining min and max values"), this);
  ...
  progress->mbar->setWhatsThis (tr ("Progress of the generation of min and max values."));
  ...
  progress->gbox = new QGroupBox (tr ("Writing GeoTIFF"), this);
  ...
  progress->gbar->setWhatsThis (tr ("Progress of the GeoTIFF writing process."));
  ...
  progress->cbox = new QGroupBox (tr ("Generating contours"), this);
  QVBoxLayout *cboxLayout = new QVBoxLayout;
  progress->cbox->setLayout (cboxLayout);
  cboxLayout->setSpacing (10);


  progress->cbar = new QProgressBar (this);
  progress->cbar->setRange (0, 100);
  progress->cbar->setWhatsThis (tr ("Progress of the contour generating process."));
  cboxLayout->addWidget (progress->cbar);


  vbox->addWidget (progress->cbox);
  ...
  *cList = checkList;
  ...
  registerField ("progress_cbar*", progress->cbar, "value");
}
