...
pfmPage::validatePage ()
{
  if (l_page_num == 1 && (pfm_file_edit->text ().isEmpty () || area_edit->text ().isEmpty ()))
    {
      QMessageBox::critical (this, tr ("pfmLoadM first page"),
...
{
  QFileDialog *fd = new QFileDialog (this, tr ("pfmLoadM Mosaic File"));
  fd->setViewMode (QFileDialog::List);
...
pfmPage::slotMapError (QProcess::ProcessError error)
{
  switch (error)
    ...
    case QProcess::FailedToStart:
      QMessageBox::critical (this, tr ("pfmLoadM map editor"), tr ("Unable to start the map editor process!"));
      break;

    case QProcess::Crashed:
      QMessageBox::critical (this, tr ("pfmLoadM map editor"), tr ("The map editor process crashed!"));
      break;

    case QProcess::Timedout:
      QMessageBox::critical (this, tr ("pfmLoadM map editor"), tr ("The map editor process timed out!"));
      break;

    case QProcess::WriteError:
      QMessageBox::critical (this, tr ("pfmLoadM map editor"), 
...
pfmPage::slotAreaMap ()
{
  if (pfm_def->name.isEmpty ())
    {
      QMessageBox::warning (this, tr ("pfmLoadM"),
...
pfmPage::slotAreaPFM ()
{
  ...
  if (pfm_def->name.isEmpty ())
    ...
    {
      QFileDialog *fd = new QFileDialog (this, tr ("pfmLoadM Open PFM Structure (area)"));
      fd->setViewMode (QFileDialog::List);
...
pfmPage::slotAreaNSEW ()
{
  if (pfm_def->name.isEmpty ())
  ...
  else
    {
      ...
      nsewD->setWindowTitle (tr ("pfmLoadM NSEW bounds"));
...
pfmPage::slotApplyNSEW ()
{
  if (north->text ().isEmpty () || south->text ().isEmpty () || west->text ().isEmpty () || east->text ().isEmpty ())
    {
      QMessageBox::warning (this, tr ("pfmLoadM NSEW bounds"), tr ("You must set all four fields or use the Cancel button."));
    }
  else
    {
	...
	{
	  QMessageBox::warning (this, tr ("pfmLoadM NSEW bounds"), tr ("Unable to open area file ") + pfm_def->area);
	}
      else
	{
