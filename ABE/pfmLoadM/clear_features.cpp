          ...
          if (strstr (ftr, ".xml"))
          ...
          else
            {
              if ((bfd_handle = binaryFeatureData_open_file (ftr, &bfd_header, BFDATA_READONLY)) < 0)
                {
                  ...
                  QMessageBox::warning (0, pfmLoadM::tr ("pfmLoadM"), pfmLoadM::tr ("Unable to open feature file\nReason: ") + msg);
                  return (NVFalse);
                }
              ...
              if (binaryFeatureData_read_all_short_features (bfd_handle, &feature) < 0)
                {
                  QString msg = QString (binaryFeatureData_strerror ());
                  QMessageBox::warning (0, pfmLoadM::tr ("pfmLoadM"), pfmLoadM::tr ("Unable to read feature records\nReason: ") + msg);
