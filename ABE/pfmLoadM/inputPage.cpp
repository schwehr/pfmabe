...
{
  QFileDialog fd (this, tr ("pfmLoadM Input files"));
...
{
  QString title = tr ("pfmLoadM Input directories, mask = ") + *inputFilter;
...
inputPage::slotPfmBrowseClicked ()
{
  ...
  QFileDialog *fd = new QFileDialog (this, tr ("pfmLoadM Input PFM Structure"));
  fd->setViewMode (QFileDialog::List);
      ...
      if (!file.isEmpty())
        {
          ...
          if ((temp_handle = open_existing_pfm_file (&open_args)) < 0)
            {
              QMessageBox::warning (this, tr ("pfmLoadM Input PFM Structure"),
